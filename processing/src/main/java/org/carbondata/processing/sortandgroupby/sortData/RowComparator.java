...
public class RowComparator implements Comparator<Object[]> {
    ...
    public int compare(Object[] rowA, Object[] rowB) {
        ...
        for (boolean isNoDictionary : noDictionaryColMaping) {
            ...
            if (isNoDictionary) {
                byte[] byteArr1 = (byte[]) rowA[IgnoreDictionary.BYTE_ARRAY_INDEX_IN_ROW
                        .getIndex()];

                ByteBuffer buff1 = ByteBuffer.wrap(byteArr1);

                // extract a high card dims from complete byte[].
                RemoveDictionaryUtil.extractSingleHighCardDims(
                        byteArr1, noDictionaryindex, noDictionaryCount, buff1);

                byte[] byteArr2 = (byte[]) rowB[IgnoreDictionary.BYTE_ARRAY_INDEX_IN_ROW
                        .getIndex()];
                ...
                // extract a high card dims from complete byte[].
                RemoveDictionaryUtil.extractSingleHighCardDims(
                        byteArr2, noDictionaryindex, noDictionaryCount, buff2);
